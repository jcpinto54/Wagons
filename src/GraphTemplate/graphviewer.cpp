...
void GraphViewer::initialize(int width, int height, bool dynamic, int port_n) {
	...
	string command = "java -jar ../src/GraphTemplate/GraphViewerController.jar";
	std::stringstream ss;
	ss << port_n;
...
GraphViewer::GraphViewer(Graph<Local *> &graph) {
    this->width = Local::getMaxX() - Local::getMinX();
    this->height = Local::getMaxY() - Local::getMinY();
    initialize(width, height, false, GraphViewer::port);
    ++GraphViewer::port;

    for (auto vertex : graph.getVertexSet()) {
        this->addNode(vertex->getInfo()->getId(), vertex->getInfo()->getX(), vertex->getInfo()->getY());
    }
    int i = 0;
    for (auto vertex : graph.getVertexSet()) {
        for (auto edge : vertex->getAdj()) {
            this->addEdge(i, vertex->getInfo()->getId(), edge.getDest()->getInfo()->getId(), EdgeType::UNDIRECTED);
            i++;
        }
    }
}
